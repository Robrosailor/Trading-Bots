...
API_KEY = "organizations/251f5945-596c-47ee-ad90-4327620bee58/apiKeys/bc0eaf4d-b414-4efd-ada7-048bb0eae74f"
...
def get_eth_balance():
    return 0.05  # example placeholder
...
def place_buy_order(price):
    if DRY_RUN:
        msg = f"[DRY RUN] BUY ETH at {price}"
        print(msg)
        send_discord(LOGS_WEBHOOK, msg)
    else:
        msg = f"Executing BUY ETH at {price}"
        print(msg)
...
def place_sell_order(price):
    if DRY_RUN:
        msg = f"[DRY RUN] SELL ETH at {price}"
        print(msg)
        send_discord(LOGS_WEBHOOK, msg)
    else:
        msg = f"Executing SELL ETH at {price}"
        print(msg)
        send_discord(ALERTS_WEBHOOK, msg)
...
def run_bot():
    ...
    while True:
        try:
            ...
            log_msg = f"ETH Price: {current_price:.4f} | EMA20: {ema:.4f} | Var: {variance:.2f}%"
            print(log_msg)
